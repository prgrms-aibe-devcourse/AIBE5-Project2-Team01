...
| 문제 | MeetBall의 해결 방향 |
| --- | --- |
| 프로젝트 정보가 흩어져 탐색 비용이 큼 | 프로젝트 목록, 검색, 목적/진행 방식/포지션/기술 스택 필터 제공 |
...
| 본인 | 로그인 사용자 자신 | `/mypage`에서 개인 정보, 찜, 지원, 최근 조회, 받은 리뷰 등 개인 데이터를 관리 |
| 타인 | 로그인 사용자가 조회하는 다른 회원 | `/people/{profileId}`에서 공개 프로필과 만든/참여/종료 프로젝트만 조회 |
...
| 기능 | 설명 |
| --- | --- |
| 프로젝트 탐색 | 프로젝트 목록 조회, 키워드 검색, 목적/진행 방식/포지션/기술 스택 필터 |
...
| 추천 프로젝트 | 로그인 사용자의 기술 스택과 포지션을 기준으로 후보군을 만들고, 질문 축/버블/카드 흐름으로 추천 |
...
| 도메인 | 구현 내용 |
| --- | --- |
| 인증과 프로필 | Google 로그인 API로 계정과 기본 프로필을 생성/조회하고, `HttpSession`에 `profileId`, `profileNickname`, `needsProfile`, Spring Security context를 저장합니다. 최초 로그인 시 온보딩으로 계정/프로필/기술 스택을 완성하고, 타인 조회는 별도 `/people/{profileId}` 공개 프로필로 분리합니다. |
| 프로젝트 탐색과 등록 | 프로젝트 목록을 페이지 단위로 조회하고, 키워드/목적/진행 방식/포지션/기술 스택 필터를 지원합니다. 등록자는 자동으로 프로젝트 `LEADER` 멤버가 됩니다. |
...
| 사람 프로필 | 프로젝트 상세의 팀장/댓글 작성자 링크에서 `/people/{profileId}`로 이동합니다. 회원만 조회 가능하며, 본인을 조회하면 `/mypage`로 이동합니다. 이메일, 찜, 지원, 최근 조회, 받은 리뷰 같은 개인 데이터는 노출하지 않습니다. |
...
flowchart LR
    ...
    P --> Q{로그인 여부}
    Q -->|비로그인| D
    Q -->|로그인| R["사람 프로필 /people/{profileId}"]
    B --> C{로그인 여부}
    C -->|비로그인| D[로그인 모달]
    C -->|로그인| E[지원 또는 찜]
    E --> F[팀장 지원자 관리]
...
프로젝트 상태는 `recruitStatus`와 `progressStatus`를 분리합니다. 지원 차단과 목록 노출에는 모집 상태를, 팀원 리뷰와 종료 프로젝트 이력에는 진행 상태를 사용합니다.

마이페이지(`/mypage`)는 로그인한 본인만 사용하는 개인 공간입니다. 다른 사용자의 프로필과 참여 이력은 프로젝트 상세 또는 댓글 작성자 링크에서 `/people/{profileId}` 사람 프로필 페이지로 확인합니다.
...
| 마이페이지 | `/mypage` | 본인 전용 개인 정보, 지원, 찜, 읽은 프로젝트, 참여 프로젝트, 리뷰 관리 |
| 사람 프로필 | `/people/{profileId}` | 회원 전용 타인 공개 프로필, 만든/참여/종료 프로젝트 조회 |
...
│   └── HomeController
├── domain
│   ├── account          # 계정 엔티티와 인증 계정 정보
│   ├── auth             # Google 로그인, 현재 사용자 조회, 로그아웃
│   ├── bookmarkedproject # 프로젝트 찜 상태 조회와 토글
│   ├── catalog         # 포지션/기술 스택 기본 카탈로그
│   ├── comment          # 댓글과 답글 조회, 작성, 수정, 삭제
│   ├── mypage           # 프로필, 지원, 찜, 참여 프로젝트, 리뷰 통합 조회
│   ├── people           # 회원 전용 타인 공개 프로필과 공개 참여 프로젝트 조회
│   ├── position         # 포지션 엔티티와 조회
│   ├── profile          # 프로필, 온보딩, 프로필 수정
│   ├── project          # 프로젝트 목록, 상세, 등록, 수정, 삭제, 멤버 관리
│   ├── projectapplication # 프로젝트 지원 생성, 조회, 상태 변경
│   ├── projectresource  # 파일 업로드, 다운로드, 링크 첨부
│   ├── recommendation   # 추천 후보군, 질문 축/버블, Mock/Gemini/Fallback 추천 흐름
│   ├── review           # 프로젝트 리뷰, 팀원 리뷰, 밋볼 지수
│   ├── techstack        # 기술 스택 엔티티와 조회
│   └── viewhistory      # 최근 읽은 프로젝트 기록
└── global
...
### Comment, Bookmark, Resource, Review
...
| `GET` | `/api/projects/{projectId}/resources` | 자료 목록 조회 |
| `POST` | `/api/projects/{projectId}/resources` | 파일 자료 업로드 |
| `POST` | `/api/projects/{projectId}/resources/links` | 링크 자료 등록 |
| `GET` | `/api/projects/{projectId}/resources/{resourceId}/download` | 파일 자료 다운로드 |
...
| Method | Endpoint | 설명 |
| --- | --- | --- |
| `GET` | `/api/mypage/account` | 계정/프로필 조회 |
| `PUT` | `/api/mypage/account` | 계정/프로필 수정 |
...
| `GET` | `/api/recommendations` | 추천 카드와 질문 축/버블 조회 |
...
마이페이지는 본인 전용 개인 공간이고, 타인 조회는 사람 프로필 페이지로 분리합니다. `/mypage?profileId=타인`처럼 접근해도 로그인 사용자는 본인 마이페이지를 유지하고, 타인 정보는 `/people/{profileId}`에서 조회하도록 유도합니다.

| 항목 | 본인 | 타인 회원 | 게스트 |
| --- | --- | --- | --- |
| `/mypage` | 조회 및 수정 가능 | 접근 불가. `/people/{profileId}`로 이동 | 로그인 필요 |
| 마이페이지 프로필 | 이메일 포함 상세 조회, 수정 가능 | 비공개 | 비공개 |
| 찜/지원/조회 이력 | 조회 가능 | 비공개 | 비공개 |
| 마이페이지 참여 프로젝트 | 조회 가능 | 비공개 | 비공개 |
| 받은 리뷰 | 조회 가능 | 비공개 | 비공개 |
| `/people/{profileId}` 공개 프로필 | 본인 접근 시 `/mypage`로 이동 | 조회 가능 | 로그인 필요 |
| 사람 프로필 참여 이력 | 본인 접근 시 `/mypage`로 이동 | 만든 프로젝트, 참여 중인 프로젝트, 종료된 프로젝트 조회 가능 | 로그인 필요 |
| 사람 프로필 개인정보 | 본인 접근 시 `/mypage`로 이동 | 이메일, 찜, 지원, 최근 조회, 받은 리뷰 미노출 | 로그인 필요 |

### 프로젝트 상태 기준

| 상태 | 의미 | 주요 사용처 |
| --- | --- | --- |
| 모집 중 | `recruitStatus=OPEN`, `progressStatus=READY/IN_PROGRESS` | 지원 가능 여부, 프로젝트 목록 표시 |
| 모집 마감 | `recruitStatus=CLOSED` | 신규 지원 차단, 프로젝트는 계속 진행 가능 |
| 프로젝트 완료 | `progressStatus=COMPLETED` | 종료 프로젝트 이력, 팀원 리뷰, 밋볼 지수 반영 |
...
로컬 H2는 테스트나 개인 개발 확인 용도의 메모리 DB입니다. 기본값은 `APP_SEED_ENABLED=true`이며, 애플리케이션을 실행할 때마다 검증용 사용자, 프로젝트, 지원, 댓글, 찜, 조회, 리뷰 데이터가 자동으로 생성됩니다. 운영 Render에서는 `APP_SEED_ENABLED=false`로 비활성화합니다.
...
로컬 DB는 메모리에만 유지되므로 애플리케이션을 종료하면 초기화됩니다. 샘플 데이터 없이 확인하려면 `APP_SEED_ENABLED=false`를 함께 지정합니다.
