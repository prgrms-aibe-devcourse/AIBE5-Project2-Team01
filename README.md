...
본인	로그인 사용자 자신	/mypage에서 개인 정보, 찜, 지원, 최근 조회, 받은 리뷰 등 개인 데이터를 관리
...
도메인별 상세
도메인	구현 내용
인증과 프로필	Google 로그인 API로 계정과 기본 프로필을 생성/조회하고, HttpSession에 profileId, profileNickname, needsProfile, Spring Security context를 저장합니다. 최초 로그인 시 온보딩으로 계정/프로필/기술 스택을 완성하고, 타인 조회는 별도 /people/{profileId} 공개 프로필로 분리합니다.
...
사람 프로필	프로젝트 상세의 팀장/댓글 작성자 링크에서 /people/{profileId}로 이동합니다. 회원만 조회 가능하며, 본인을 조회하면 /mypage로 이동합니다. 이메일, 찜, 지원, 최근 조회, 받은 리뷰 같은 개인 데이터는 노출하지 않습니다.
서비스 흐름
...
팀원 리뷰
마이페이지 밋볼 지수 반영
프로젝트 상태는 recruitStatus와 progressStatus를 분리합니다. 지원 차단과 목록 노출에는 모집 상태를, 팀원 리뷰와 종료 프로젝트 이력에는 진행 상태를 사용합니다.
...
마이페이지와 타인 프로필
마이페이지는 본인 전용 개인 공간이고, 타인 조회는 사람 프로필 페이지로 분리합니다. /mypage?profileId=타인처럼 접근해도 로그인 사용자는 본인 마이페이지를 유지하고, 타인 정보는 /people/{profileId}에서 조회하도록 유도합니다.
...
받은 리뷰	조회 가능	비공개	비공개
/people/{profileId} 공개 프로필	본인 접근 시 /mypage로 이동	조회 가능	로그인 필요
사람 프로필 참여 이력	본인 접근 시 /mypage로 이동	만든 프로젝트, 참여 중인 프로젝트, 종료된 프로젝트 조회 가능	로그인 필요
사람 프로필 개인정보	본인 접근 시 /mypage로 이동	이메일, 찜, 지원, 최근 조회, 받은 리뷰 미노출	로그인 필요
프로젝트 상태 기준
상태	의미	주요 사용처
...
환경 설정
기본 프로파일은 prod입니다. 로컬에서 실행할 때는 반드시 SPRING_PROFILES_ACTIVE=local을 지정합니다.
...
Google 로그인은 기본 Client ID로 동작합니다. 운영 Client ID를 별도로 분리해야 할 때만 Render Environment에 GOOGLE_CLIENT_ID를 지정합니다.
