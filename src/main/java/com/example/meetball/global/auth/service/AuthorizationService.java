...
public class AuthorizationService {
     ...
     */
    public ProjectDetailRole getProjectDetailRole(Profile currentProfile, Project project) {
        ...
        // 2. 프로젝트 멤버 여부 확인
        Optional<ProjectParticipant> participant = projectParticipantRepository.findByProjectAndProfile(project, currentProfile);
        ...
        // 3. 팀장 vs 팀원 구분
        String role = participant.get().getRole();
     ...
     */
    public MyPageAccess getMyPageAccess(Profile currentProfile, Profile profileUser) {
        // 로그인하지 않았거나, 로그인한 ID와 프로필 ID가 다르면 타인(VISITOR)
        if (currentProfile == null || !currentProfile.getId().equals(profileUser.getId())) {
